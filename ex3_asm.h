...
#define VERILOG_READ_MEM_SEPARATOR " "
	void PrintMemoryWord(Memory::Word * m, FILE * fp, int addr, int printMode)
	{
		...
		if(fp){
			...
			if(verilogFlag){
				if(showProgram)			{ fprintf(fp, "@%03x%s%08x\t///", addr, VERILOG_READ_MEM_SEPARATOR, m->value);}
				else if(showMonitor)	{ fprintf(fp, "%1x%03x%08x\t///\t", (insn->type == EX3_InsnSet::NON_INSN), addr, m->value);}
				else					{ fprintf(fp, "%1x%03x%08x\t///\t", 0, addr, m->value);}
			}
			...
			if(insn->type == EX3_InsnSet::MEM_INSN){
				...
				fprintf(fp, "%04x [%08x]: %s %03x %s (%*s)", addr, m->value, insn->name, addrValue, 
					(mem->IsIndirectMemoryAccess(m)) ? "I" : " ", label.maxLabelLength, (target_lb) ? target_lb->name : "???");
			}
			else if(insn->type == EX3_InsnSet::REG_INSN)
			{ fprintf(fp, "%04x [%08x]: %s%*s", addr, m->value, insn->name, label.maxLabelLength + 9, "");}
			else if(m->status != 0){
				const char * field = (m->value >= 0x20 && m->value <= 0x7e) ? "%04x [%08x]: (%5d:  '%c')%*s" : "%04x [%08x]: (%5d:%5d)%*s";
				fprintf(fp, field, addr, m->value, (signed int) m->value, m->value, label.maxLabelLength + 1, "");
			}
			else if(m->value != 0)
			{ EMIT_MESSAGE_3(fprintf, fp, "%04x [%08x]: ERROR!!! this memory location is unused but has non-zero value!!!\n", addr, m->value);}
