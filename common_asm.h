...
public:
	...
	public:
		...
		void Set(int id, const char * n, int t, int showMem, void (* op)(CPU *), unsigned int c){
...
public:
	...
	int ParseLabel(int passNum, const char ** p, int & addr){
		...
		if(labelLength > 0){
			if(cpu->label.annotation.AddAnnotation(*p, labelLength, (unsigned int) addr)){}
			else if(passNum == 1){	///	create label
				if(cpu->label.AddLabel(*p, labelLength, (unsigned int) addr) == 0){ PrintErrorLocation(); return -1;}
			}
